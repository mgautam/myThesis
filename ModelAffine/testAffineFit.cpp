...
void testAffineModel (void) {
	...
	initial.scores = new double[initial.Number_of_Coordinates];	
	...
	final.scores = new double[final.Number_of_Coordinates];	

	cout << "Test Vectors:" << endl;
	for (int index = 0; index < NUMBER_OF_PIXELS; index++) {
		initial.x[index] = index;// + 256;
		initial.y[index] = (double)rand()/(double)RAND_MAX * NUMBER_OF_PIXELS;// + 256;
		initial.scores[index] = rand ();
		...
		final.scores[index] = initial.scores[index];
